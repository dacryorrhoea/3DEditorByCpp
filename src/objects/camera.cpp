...
std::vector<int> Camera::selectVertex(int x, int y, Object* obj) {
    ...
    try {
        ...
        getVerticesCamSpace(verticesWorld, verticesCamSpace);
        ...
        for (const Face& face : faces) {
            if (face.f1 == bestIdx || face.f2 == bestIdx ||
                face.f3 == bestIdx) {
                if (std::find(result.begin(), result.end(), face.f1)
                    == result.end()) {
                    result.push_back(face.f1);
                }
                if (std::find(result.begin(), result.end(), face.f2)
                    == result.end()) {
                    result.push_back(face.f2);
                }
                if (std::find(result.begin(), result.end(), face.f3)
                    == result.end()) {
...
void Camera::toProjectingScene(
    ...
    try {
        ...
        for (Object* obj : objects) {
            ...
            FaceContainer &faces = mesh->getFaces();
            for (Face &face : faces) {
                if (verticesCamSpace[face.f1].z <= nearPlane ||
                    verticesCamSpace[face.f2].z <= nearPlane ||
                    verticesCamSpace[face.f3].z <= nearPlane) {
                    continue;
                }
                ...
                if (normal.dot(normal) < 1e-8f) continue;
                normal.normalized();
